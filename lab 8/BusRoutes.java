...
class BusRoutes {
   ...
   * @param services The set of bus services between the two stops stored within a 
   *     CompletableFuture.
   */
  ...
  /**
   * Returns a CompletableFuture containing a string describing the bus route.
   * @return A CompletableFuture containing the first line contains the query information:
   *     bus stop id and search string. The remaining line contains the bus services and 
   *     matching stops served.
   */
  public CompletableFuture<String> description() {
    String result = "Search for: " + this.stop + " <-> " + name + ":\n";
    result += "From " +  this.stop + "\n";
    CompletableFuture<String> resultCF = CompletableFuture.completedFuture(result);
 
    for (BusService service : services.keySet()) {
      CompletableFuture<String> next = services.get(service).thenApply(stops -> 
          describeService(service, stops)); 
      resultCF = resultCF.thenCombine(next, (x, y) -> x + y);
    }
    return resultCF;
  }

  /**
   * Returns a string representation a bus service and its matching stops.
