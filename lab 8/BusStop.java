...
class BusStop {
  ...
  /**
   * Returns the set of bus services that serve this bus stop as
   * a set encapsulated within a CompletableFuture.  Query the web server.
   * @return A CompletableFuture containing a set of BusService that serve this bus stop.
   */
  public CompletableFuture<Set<BusService>> getBusServices() {
    // Thread.sleep(200);
    // bus services that visit this stop

    CompletableFuture<String> busServicesCF = BusAPI.getBusServicesAt(stopId);
    return busServicesCF.thenApply(busServicesString -> {
      Scanner sc = new Scanner(busServicesString);
  ...
  /**
   * Checks if this bus stop equals to another bus stop -- two bus
